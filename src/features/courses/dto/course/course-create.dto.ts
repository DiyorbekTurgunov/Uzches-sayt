...
export class CourseCreateDto {
    @ApiProperty() @IsInt() @Type(() => Number) authorId: number;
    @ApiProperty() @IsInt() @Type(() => Number) categoryId: number;
    @ApiProperty() @IsInt() @Type(() => Number) languageId: number;
    @ApiProperty() @IsInt() @Type(() => Number) difficultyId: number;
    @ApiProperty() @IsString() @MaxLength(128) title: string;
    @ApiProperty() @IsString() @MaxLength(128) image: string;
    @ApiProperty() @IsNumber({ maxDecimalPlaces: 2 }) @Min(0) @Type(() => Number) price: number;
